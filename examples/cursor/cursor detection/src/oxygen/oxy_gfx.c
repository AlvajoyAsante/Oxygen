...
struct oxy_FrameManager_t oxy_FrameManager;
...
/**
 * This function returns a the set transparent of "gfx" library.
 * It's not needed but it's fun to have :).
 */
void oxy_TransparentGIF(gfx_sprite_t *frames[], uint8_t frame_amount, uint16_t x, uint8_t y)
{
	gfx_TransparentSprite(frames[oxy_ReturnFrame(frame_amount)], x, y);
}

void oxy_GIF(gfx_sprite_t *frames[], uint8_t frame_amount, uint16_t x, uint8_t y)
{
	gfx_Sprite(frames[oxy_ReturnFrame(frame_amount)], x, y);
}

uint8_t oxy_ReturnFrame(uint8_t frame_amount)
{
	if (oxy_FrameManager.frame < frame_amount)
	{
		return oxy_FrameManager.frame;
	}
	else
	{
		return 0;
...
void oxy_UpdateGIF(uint16_t frame_change_time)
{
	if (oxy_FrameManager.time != frame_change_time)
	{
		oxy_FrameManager.time++;
	}
	else
	{
		oxy_FrameManager.frame++;
		oxy_FrameManager.time = 0;
	}
}