...
/**
 * This function is used to save all Oxygen's data and systems.
 */
...
/**
 * This function is used to load all oxygen data including file system.
 */
bool oxy_LoadAll(void)
{
	...
	if ((slot = ti_Open(OXY_APPVAR_NAME, "r")))
	{
		...
		/* Notify System */
		if (ti_Read(&oxy_notify_system, sizeof(struct oxy_notify_system_t), 1, slot))
		{ // All ways check for the system before reading
...
void oxy_InitAll(void)
{
	...
	/* User System */
	oxy_NewUser("USERNAME", NULL, 0); // Creates a user
