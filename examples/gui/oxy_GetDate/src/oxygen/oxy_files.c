			...
			if (oxy_GetFileType(curr_file->type) == TI_PPRGM_TYPE)
			{
				for (int r = 0; r < 20; r++)
				{
					...
					if (data_pos == 16)
					{
						...
						curr_file->description = (char *)(curr_file->icon + 258);
					}
				...
				if (!memcmp(search, ti_GetDataPtr(slot), 6))
				{
					...
					for (int r = 0; r < 256; r++)
					{
						...
						if (data_pos != tString)
						{
							...
							// curr_file->icon->data[r] = (uint8_t)palette[strtol(temp, NULL, 16)];
						}
						else
						{
							free(curr_file->icon);
...
void oxy_EditFile(const char *name)
{
	ti_var_t slot;

	oxy_SaveAll();

	if ((slot = ti_OpenVar(name, "r", TI_PRGM_TYPE)))
	{
