...
void oxy_AlertNotify(int index)
{
	...
	if (curr_notify->icon != NULL){ // icon detected
		xprint = (LCD_WIDTH - 205)/2;
		oxy_FillRoundRectangle(xprint, yprint, 205, 40, 0); // Clears area
		
		gfx_SetColor(255);
		oxy_FillRoundRectangle(xprint, yprint + 1, 38, 38, 0); //Icon box
